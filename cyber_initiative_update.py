...
class CyberScanner:
    ...
    def generate_pdf_report(self):
        ...
        try:
            ...
            # Return PDF as bytes
            pdf_output = pdf.output(dest='S')  # Returns bytearray
            self._log(f"PDF output type before conversion: {type(pdf_output)}, length: {len(pdf_output)}")
            pdf_bytes = bytes(pdf_output)  # Convert to bytes for Streamlit
            self._log(f"PDF output type after conversion: {type(pdf_bytes)}, length: {len(pdf_bytes)}")
            return pdf_bytes
        except Exception as e:
